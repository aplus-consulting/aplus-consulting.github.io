...
CONFIG = {
  ...
  'paper_ext' => "md",
  'software' => File.join(SOURCE, "software/_posts"),
  'software_ext' => "md",
...
# Usage: rake software title="Software Title" [date="2015-05-01"] [tags=[tag1,tag2]]
desc "Begin a new software in #{CONFIG['software']}"
task :software do
  abort("rake aborted: '#{CONFIG['software']}' directory not found.") unless FileTest.directory?(CONFIG['software'])
  title = ENV["title"] || "new-software"
  tags = ENV["tags"] || "[]"
  category = ENV["category"] || ""
  category = "\"#{category.gsub(/-/,' ')}\"" if !category.empty?
  slug = title.downcase.strip.gsub(' ', '-').gsub(/[^\w-]/, '')
  ...
  filename = File.join(CONFIG['software'], "#{date}-#{slug}.#{CONFIG['software_ext']}")
  if File.exist?(filename)
    abort("rake aborted!") if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
  end

  puts "Creating new software: #{filename}"
  open(filename, 'w') do |post|
    post.puts "---"
    post.puts "layout: software"
    post.puts "title: \"#{title.gsub(/-/,' ')}\""
    post.puts 'description: ""'
    post.puts "category: software"
    post.puts "tags: #{tags}"
    post.puts "---"
    post.puts "{% include JB/setup %}"
  end
